...
how many years 'Y' this sum 'P' has to be kept in the bank in order for it to amount to a desired 
sum of money 'D'.
...
function calculateYears(principal, interest, tax, desired) {
    ...
    let interestMinusTax = interestGained - (interestGained * tax);
    let yearlyTotal = principal + interestMinusTax;
    let counter = 1;
    while (yearlyTotal < desired) {
      counter++
      interestGained = yearlyTotal * interest;
      interestMinusTax = interestGained - (interestGained * tax);
      yearlyTotal = yearlyTotal + interestMinusTax;
    }
 return desired === principal? 0 : counter; 
}

// A shorter, smarter one (not mine):
